...
gen y_linear = cond(x>0.8,5*(x-0.8),0) + rnormal()/10
...
graph export convex-resp1.pdf, replace
...
graph export convex-resp2.pdf, replace
...
graph export convex-resp3.pdf, replace
...
tw ///
	...
	text(-0.4 0.25 "comparison", color(vermillion) place(n) size(large)) ///
	text(-0.4 0.75 "treatment", color(sea) place(n) size(large)) 
graph export convex-resp4.pdf, replace
...
tw ///
	...
	text(-0.4 0.25 "comparison", color(vermillion) place(n) size(large)) ///
	text(-0.4 0.75 "treatment", color(sea) place(n) size(large)) 
graph export convex-resp5.pdf, replace
...
di `mybeta'
tw ///
	(lpolyci z x if x>0.01, fcolor(turquoise%20) lcolor(turquoise*1.2)  lwidth(thin)) ///
	(scatter y_linear x, mcolor(vermillion%24) msymbol(o)) ///
	(lfit y_linear x, lcolor(sea) lwidth(thin)) , ///
	legend(off) ///
	xlabel(0(0.25)1, grid gmin gmax glwidth(thin) glcolor(gs14)) ///
	...
	text(-0.4 0.25 "comparison", color(reddish) place(n) size(large)) ///
	text(-0.4 0.75 "treatment", color(reddish) place(n) size(large)) ///
	text(1.375 0.05 "{&beta}{subscript:TRUE} = `meanz'", color(turquoise*1.2) place(e) size(large)) ///
	text(1.125 0.05 "{&beta}{subscript:OLS} = `mybeta'", color(turquoise*1.2) place(e) size(large)) 
graph export convex-resp-explain.pdf, replace
