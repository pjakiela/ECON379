...
bys dhsclust:  egen tempvar = mean(sba) if post==0
bys dhsclust:  egen pre_sba = max(tempvar)
drop tempvar

bys dhsclust:  egen tempvar = mean(sba) if post==1
bys dhsclust:  egen post_sba = max(tempvar)
drop tempvar

gen delta_sba = post_sba - pre_sba
...
tw ///
	...
	(lpolyci delta_sba pre_mean if ctag==1 & pre_mean<=0.81, color(sea%32)) ///
	(scatter delta_sba pre_mean if ctag==1 & pre_mean<=0.81, mcolor(vermillion%24) msymbol(o)) ///
	(lpoly delta_sba pre_mean if ctag==1 & pre_mean<=0.81, color(sea)), ///
	...
	ylabel(-0.75(0.25)0.75, grid gmin gmax glwidth(vthin) glcolor(gs12)) ///
	ytitle("Change in use of SBA after the ban (percent of births)" " ", size(small)) 

graph export sba-change-scatter.pdf, replace
...
tw ///
	...
	text(-0.625 0.2357143 "less", color(vermillion) place(w) size(large)) ///
	text(-0.625 0.3357143 "more", color(vermillion) place(e) size(large))
